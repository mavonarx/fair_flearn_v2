...
class Model(object):    
    ...
    def create_model(self, q, optimizer):
        ...
        features = tf.compat.v1.placeholder(tf.float32, shape=[None, 99], name='features')
        ...
        loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(labels=labels, logits=logits)  # logits must be one-hot encoding; labels must be just scalars
